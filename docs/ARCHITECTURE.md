...
本仓库现在是一个小型 monorepo，分成两个已经落地的目录：
...
2. `GET /libraries/{libraryID}/ops?after=<cursor>`。
3. 把返回的远端 committed events append 到本地 `operation_ledger`，状态为 `acknowledged`，并保存每条事件的服务端 `global_seq`。
...
4. 生成稳定 `opID` 的 snapshot 事件：
   ...
   - thumbnail/preview derivative declaration
...
S3 对象 key 在 bootstrap 中按 library/asset/role/content hash 派生。未来正式后端应由 control-plane 签发上传/下载 URL，客户端不应靠猜 key 作为权限边界。
...
```json
{
  ...
  "implemented_runtimes": ["macos_app", "control_plane_devtest"],
