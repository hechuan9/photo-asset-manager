...
struct SidebarUXTests {
    ...
    @Test func photoImportCopiesExternalFoldersIntoIndexedTargets() throws {
        ...
        let operations = try sourceFile("Sources/PhotoAssetManager/FileOperations.swift")
        let models = try sourceFile("Sources/PhotoAssetManager/Models.swift")
        let planningBody = functionBody(named: "beginPhotoImportPlanning", in: store)
        let executeBody = functionBody(named: "executePhotoImport", in: store)
        let runJobBody = functionBody(named: "runPhotoImportJob", in: operations)

        #expect(models.contains("struct PhotoImportTarget"))
        #expect(models.contains("struct PhotoImportPlanItem"))
        #expect(models.contains("struct PhotoImportPlan"))
        #expect(models.contains("struct PhotoImportJob"))
        #expect(models.contains("struct PhotoImportItem"))
        #expect(models.contains("var majorPhase = \"\""))
        #expect(content.contains("struct AppTopToolbar"))
        #expect(content.contains("struct PhotoImportDialog"))
        #expect(store.contains("@Published var isPhotoImportDialogPresented"))
        #expect(store.contains("func beginPhotoImport()"))
        #expect(store.contains("@Published var photoImportProgress: PhotoImportProgressReport?"))
        #expect(store.contains("resumeInterruptedPhotoImportIfNeeded()"))
        #expect(content.contains("struct PhotoImportProgressDialog"))
        #expect(content.contains("progress.majorPhase"))
        #expect(content.contains("Label(\"导入照片\", systemImage: \"square.and.arrow.down\")"))
        #expect(content.contains("PhotoImportDialog("))
        #expect(content.contains("library.importPhotoFolder(configuration: configuration)"))
        #expect(content.contains("PhotoImportFolderRow"))
        #expect(content.contains("RAW 文件夹"))
        #expect(content.contains("目标 RAW 文件夹"))
        #expect(store.contains("func chooseImportDirectory(message: String) -> URL?"))
        #expect(store.contains("rememberPhotoImportConfiguration"))
        #expect(store.contains("func restoredPhotoImportURL(for path:"))
        #expect(models.contains("struct PhotoImportPreferences"))
        #expect(database.contains("photo_import_preferences"))
        #expect(content.contains("restoreSavedPathsIfNeeded"))
        #expect(database.contains("CREATE TABLE IF NOT EXISTS photo_import_jobs"))
        #expect(database.contains("CREATE TABLE IF NOT EXISTS photo_import_items"))
        #expect(database.contains("func createPhotoImportJob"))
        #expect(database.contains("func unfinishedPhotoImportJob"))
        #expect(database.contains("func completePhotoImportItem"))
        #expect(database.contains("func markInterruptedPhotoImportJobs"))
        #expect(planningBody.contains("majorPhase: \"导入规划\""))
        #expect(planningBody.contains("FileOperations().buildPhotoImportPlan"))
        #expect(planningBody.contains("database.createPhotoImportJob"))
        #expect(executeBody.contains("majorPhase: \"执行导入\""))
        #expect(executeBody.contains("FileOperations().runPhotoImportJob"))
        #expect(executeBody.contains("database.markSourceDirectoryScanned(path: job.targetPath)"))
        #expect(!executeBody.contains("scanner.scanDirectory"))
        #expect(operations.contains("func buildPhotoImportPlan"))
        #expect(operations.contains("routesToHasselbladRaw"))
        #expect(operations.contains("func runPhotoImportJob"))
        #expect(runJobBody.contains("persistImportedFile"))
        #expect(runJobBody.contains("completePhotoImportItem"))
        #expect(runJobBody.contains("copyImportedItem"))
        #expect(!runJobBody.contains("removeItem(at: source)"))
        #expect(!runJobBody.contains("emptySourceDirectoryTree"))
    }
